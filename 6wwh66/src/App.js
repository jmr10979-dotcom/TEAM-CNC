...
const USERS = [
  ...
  { name: "SUPERVISOR", pass: "199419" },
  { name: "CNC T1", pass: "101010" },
  { name: "CNC T2", pass: "202020" },
  { name: "SANDING T1", pass: "303030" },
  { name: "SANDING T2", pass: "404040" },
];
...
const App = () => {
  ...
  const [orders, setOrders] = useState(() => {
    try {
      const saved = localStorage.getItem("prod_v25_inapp_notify");
  ...
  const [clientList, setClientList] = useState(() => {
    try {
      const saved = localStorage.getItem("prod_client_list_v2");
  ...
  const [notifHistory, setNotifHistory] = useState(() => {
    try {
      const saved = localStorage.getItem("prod_notif_history");
      return saved ? JSON.parse(saved) : [];
    } catch (e) {
      return [];
    }
  ...
  // Estados visuales
  const [notification, setNotification] = useState(null); // Banner flotante
  const [showNotifPanel, setShowNotifPanel] = useState(false); // Panel desplegable

  useEffect(() => {
    localStorage.setItem("prod_v25_inapp_notify", JSON.stringify(orders));
  }, [orders]);
  useEffect(() => {
    localStorage.setItem("prod_client_list_v2", JSON.stringify(clientList));
  }, [clientList]);
  useEffect(() => {
    localStorage.setItem("prod_notif_history", JSON.stringify(notifHistory));
  }, [notifHistory]);
  ...
  const sendNotification = (title, body) => {
    ...
    setNotifHistory((prev) => [newEntry, ...prev].slice(0, 50)); // Guardamos las ultimas 50
  };
  ...
  const handleAddClient = () => {
    ...
    if (name) {
      ...
      if (!clientList.includes(upperName)) {
        const newList = [...clientList, upperName].sort();
  ...
  const addOrder = (e) => {
    ...
    sendNotification(
      "Nueva Orden",
      `#${newOrderNum} creada por ${currentUser}`
    );
  };
  ...
  const saveEditedOrder = () => {
    ...
    const updatedOrders = orders.map((o) => {
      if (o.id !== editOrderModal.id) return o;
      return {
        ...o,
  ...
  const confirmBatchAdvance = () => {
    ...
    const nextLabel = stageConfig ? stageConfig.label : "ESTADO";
    sendNotification(
      "Status Actualizado",
      `Orden #${orderNumber} movida a ${nextLabel}`
    );
  };
  ...
  const deleteOrder = (id) => {
    ...
    if (!canDelete) return;
    if (window.confirm("¿ELIMINAR ORDEN COMPLETA?")) {
      const ord = orders.find((o) => o.id === id);
      setOrders(orders.filter((o) => o.id !== id));
      sendNotification("Orden Eliminada", `Orden #${ord?.orderNum} borrada`);
    }
  };
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-slate-100 overflow-hidden font-sans text-slate-800">
      ...
      {/* HEADER FIJO */}
      <div className="flex-none z-40 bg-white shadow-md relative">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              ...
              {notifHistory.length > 0 && (
                <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[9px] w-4 h-4 flex items-center justify-center rounded-full font-bold border-2 border-slate-800">
        ...
        {/* PANEL DESPLEGABLE NOTIFICACIONES */}
        {showNotifPanel && (
          <div className="absolute top-10 right-2 w-72 bg-white rounded-lg shadow-2xl border border-slate-200 z-50 overflow-hidden animate-in slide-in-from-top-2">
            ...
            <div className="max-h-64 overflow-y-auto p-0">
              {notifHistory.length === 0 ? (
                ...
                notifHistory.map((notif) => (
                  <div
                    key={notif.id}
                    className="p-3 border-b border-slate-50 hover:bg-blue-50 transition-colors"
                  >
                    ...
                    <p className="text-[8px] text-slate-300 mt-1 text-right">
                      {notif.date}
                    </p>
                  </div>
                ))
      ...
      {notification && (
        <div className="fixed top-4 left-4 right-4 bg-slate-900/95 backdrop-blur text-white p-3 rounded-xl shadow-2xl z-[100] animate-in slide-in-from-top-10 duration-300 flex items-center gap-3 border-l-4 border-green-500">
      ...
      {/* CONTENIDO SCROLL */}
      <div
        className="flex-1 overflow-y-auto p-3 pb-24"
